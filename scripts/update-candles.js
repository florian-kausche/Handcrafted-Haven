...
async function updateCandleProducts() {
  try {
    await mongoose.connect(MONGODB_URI);
    console.log('Connected to MongoDB');
    
    // Delete all existing Candle products
    console.log('Removing old candle products...');
    const result = await Product.deleteMany({ category: 'Candles' });
    console.log(`✓ Deleted ${result.deletedCount} old candle products`);
    
    // Now add the updated candles
    console.log('Adding updated candle products...');
    ...
    console.log('\nAll candle products updated successfully!');
    await mongoose.connection.close();
    process.exit(0);
  } catch (error) {
    console.error('Error updating candle products:', error);
    process.exit(1);
  }
}

updateCandleProducts();
